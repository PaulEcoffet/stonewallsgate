...
def get_events(scene_name):
    ...
    with open(os.path.join(current_folder, "events.json"), "r",
        ...
        for json_event in json_events:
            ...
            event.dialogs = get_dialogues(current_folder, json_event['dialogs'])
...
def get_dialogues(current_folder, ref_dialogues):
    ...
    with open(
        ...
        dialogues = [Dialog() for dct in list_dct]
        for i, dct in enumerate(list_dct):
            dialogues[i].character = dct['character']
            dialogues[i].message = dct['message']
    return dialogues
