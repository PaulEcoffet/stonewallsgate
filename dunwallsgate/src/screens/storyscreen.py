...
class StoryScreen():
    ...
    def show_dialogue(self, *args):
        self.message = self.game.cache.next_panel()
        if self.message:
            self.choices = self.ask_choices()
            self.new_portraits = self.set_portraits()
            self.purge_textbox()
            self.dialogue_box.image.blit(self.message["img_txt"], (10,10))
            self.message = None
        else:
    ...
    def set_portraits(self):
        ...
        characs = []
        for _type in self.message:
            if self.message[_type] and _type in ["transmitter", "receiver"]:
                characs.append(self.game.cache.portraits[(self.message[_type], _type)])
                if _type == "transmitter" and (self.left_charac != self.message["receiver"] or not self.message["receiver"]):
                    self.left_charac = self.message[_type]
                if self.message[_type] == self.left_charac:
    ...
    def ask_choices(self):
        ...
        if self.message["choices"]:
            for i, choice in enumerate(self.message["choices"]):
                ...
                self.eventmanager.on_click_on(choices_sprite[-1], (lambda trigger, params: lambda x: self.game.game_event.game_triggers[trigger](params))(choice["trigger_cat"], choice["text"])) #choices, params : lambda truc : self.game.game_event.game_triggers[choice["trigger"]](choice["params"]))
